...
import {AlertDialogContent, AlertDialogHeader, AlertDialogTitle, AlertDialogDescription, AlertDialogFooter, AlertDialogCancel, AlertDialogAction} from './ui/alert-dialog'
...
export default function AddNewUserExerciseModal() {
  ...
  const mutationAddCustomExercise = useMutation({
    ...
    mutationFn: (options: optionsObj) => addCustomExercise(options.exerciseName, options.group, options.userEmail),
    onMutate: async () => {
  ...
  function handleAddExercise() {
    if(muscleValue && inputValue) {
      if(session) {
        const uppercaseValue = muscleValue[0].toUpperCase() + muscleValue.slice(1)
    ...
    <AlertDialogContent className=''>
      <AlertDialogHeader>
        ...
        <div>
          <div className='mb-4'>
            <p>Name:</p>
            <Input onChange={(e) => handleChange(e)} value={inputValue} className='mt-2'/>
          </div>
          <div>
            <p>Category:</p>
            <Popover open={open} onOpenChange={setOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  {muscleValue
                    ? muscles.find((muscle) => muscle.value === muscleValue)?.label
                    : "Select muscle..."}
                    ...
                    {muscles.map((muscle) => (
                      <CommandItem
                        ...
                        onSelect={(currentValue) => {
                          setMuscleValue(currentValue === muscleValue ? "" : currentValue)
                          setOpen(false)
                        }}
                      >
                        <Check
                          className={cn(
                            "mr-2 h-4 w-4",
                            muscleValue === muscle.value ? "opacity-100" : "opacity-0"
                          )}
                        />
                        {muscle.label}
                      </CommandItem>
      ...
      <AlertDialogFooter>
        <AlertDialogCancel onClick={() => setMuscleValue('')}>Cancel</AlertDialogCancel>
        <AlertDialogAction onClick={handleAddExercise} disabled={muscleValue === '' || inputValue === ''}>Add Exercise</AlertDialogAction>
      </AlertDialogFooter>
    </AlertDialogContent>
  )
}
