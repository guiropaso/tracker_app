...
export default function Page() {
  ...
  useEffect(() => {
    if (data && (data as Workout).workoutId) {
      ...
      if (exNameParam) {
        router.push(
          `${path}?workoutId=${workout.workoutId}&exName=${exNameParam}`
        );
      } else {
  ...
  return (
    <div
      className={twMerge(
        (!workoutIdParam || (!exNameParam && uniqueExercises?.length === 0)) &&
          "grid grid-rows-2 h-screen"
      )}
    >
      <div>
      ...
      {session &&
      workoutIdParam &&
      uniqueExercises !== undefined &&
      data &&
      Object.keys(data).length > 0 &&
      (data as Workout).workoutId ? (
        <TodaysWorkout
          workout={data as Workout}
          exNameParam={exNameParam}
          workoutParam={workoutIdParam}
          uniqueExercises={uniqueExercises}
        />
      ) : (
